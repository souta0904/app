...
bool Core::Initialize( uint32_t windowWidth, uint32_t windowHeight, const std::wstring& windowTitle )
{
    // ウィンドウの作成
    mWindow = &Window::GetInstance();
    if( !mWindow->Create( windowWidth, windowHeight, windowTitle ) )
    {
...
void Core::Run()
{
    ...
    while( !mWindow->ProcessMessage() )
    {
        ...
        renderer.ExecuteCulling( cmdList );
